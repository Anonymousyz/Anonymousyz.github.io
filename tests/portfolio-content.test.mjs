...
test('首页项目固定为一个主案例和两份项目记录', () => {
  ...
  assert.deepEqual(demos.map((item) => item.kind), ['内部功能版本 · 界面快照', '受控演示版 · 界面快照']);
...
test('项目页包含三个确认项目且不使用能力证明标签', async () => {
  ...
  assert.match(source, /公开内容/);
  assert.match(source, /未公开内容/);
...
test('两份数字化项目以脱敏原型快照说明流程与公开边界', () => {
  ...
  for (const project of prototypeProjects) {
    assert.match(project.prototypeShowcase.status, /界面快照/);
    assert.ok(project.prototypeShowcase.intro.length > 0);
    assert.equal(
      project.prototypeShowcase.images.length,
      project.id === 'industrial-digital-public-service-platform' ? 1 : 2
    );
...
test('工业技术改造项目以一张公共门户首页呈现', async () => {
  const source = await readFile(new URL('../src/pages/projects.astro', import.meta.url), 'utf8');
  ...
  assert.equal(industrialProject.prototypeShowcase.presentation, undefined);
  assert.equal(industrialProject.prototypeShowcase.images.length, 1);
  assert.equal(industrialProject.prototypeShowcase.images[0].label, '公共门户首页');
  assert.match(industrialProject.prototypeShowcase.images[0].boundary, /不含真实组织、项目、账号或运行地址/);
  assert.equal(oralCareProject.prototypeShowcase.images.length, 2);
  assert.doesNotMatch(source, /featured-switcher|data-prototype-switcher|role="tablist"|role="tabpanel"/);
  assert.doesNotMatch(css, /project-prototype-showcase__(switcher|tab|viewport)/);
...
test('关于页用两条主线说明工作范围', async () => {
  ...
  assert.match(about, />从研究到建设</);
  assert.match(about, />怎样处理这些工作</);
